...
export default function Dashboard() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setSensorData((prev) => ({
        ...prev,
        // KRITIKAL FIX: Tinanggal ang update logic sa waterTemp, ph, at dissolvedO2
        waterLevel: Math.min(100, Math.max(70, prev.waterLevel + (Math.random() - 0.5) * 2)),
        // Flow Rate is part of System Metrics, but since we are simulating, we update this too for demo effect
        waterFlow: Number.parseFloat((4.5 + (Math.random() * 0.3 - 0.15)).toFixed(2)),
        humidity: Number.parseFloat((65 + (Math.random() * 2 - 1)).toFixed(1)),
        ammonia: Number.parseFloat((0.3 + (Math.random() * 0.1 - 0.05)).toFixed(2)),
      }))
    }, 3000)
    return () => clearInterval(interval)
  }, [])

  // (Existing handleControlChange function)

  // Modal Components (Fixed: Removed React.FC for correct local function declaration)
  const ControlsModal = () => {

// =================================================================
<<<<<<< HEAD
// 🏡 Dashboard.tsx (e.g., app/dashboard/page.tsx)
=======
// 🚀 Dashboard.tsx
>>>>>>> parent of b52b34a (k n aba)
// =================================================================
"use client"
...
interface ControlState { pump: boolean; fan: boolean; phAdjustment: boolean; aerator: boolean; } // Subset for Dashboard Quick Controls
...
// --- Custom Hook Logic (Shared State Management) ---
const INITIAL_CONTROLS_FULL: SystemControls = { pump: true, fan: false, phAdjustment: true, aerator: true, growLight: true, }
const INITIAL_THRESHOLDS: ThresholdState = { waterTemp: { min: 20, max: 26 }, ph: { min: 6.5, max: 7.5 }, dissolvedO2: { min: 5, max: 8 }, ammonia: { min: 0, max: 0.5 }, }
const localStorageKey = 'aquaponics_settings_state';
...
>>>>>>> parent of ede362e (grow lights in quick controls)
  }
...
const SensorCard: React.FC<SensorCardProps> = ({ icon: Icon, title, value, unit, min, max, color }) => {
  const status = getThresholdStatus(value, min, max); const percentage = calculatePercentage(value, min, max); return (<div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow"><div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><div className={`p-2.5 rounded-lg ${color}`}><Icon className="w-4 h-4 text-white" /></div><span className="text-sm font-semibold text-gray-700">{title}</span></div><div className={`w-2.5 h-2.5 rounded-full ${getStatusColor(status)} animate-pulse`}></div></div><div className="mb-3"><div className="text-2xl font-bold text-gray-900">{value.toFixed(1)}<span className="text-xs text-gray-500 ml-1 font-normal">{unit}</span></div></div><div className="w-full h-1.5 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full transition-all ${getStatusColor(status)}`} style={{ width: `${Math.min(100, Math.max(0, percentage))}%` }}></div></div></div>)
};
...
<<<<<<< HEAD
  // Sync localControls state when controls from useAquaponicsSettings change, 
  // especially when modal opens or settings page forces a change.
  useEffect(() => {
    // Sync localControls when modal opens
  ...
  }, [showControlsModal, controls.pump, controls.fan, controls.phAdjustment, controls.aerator]);
=======
...
>>>>>>> parent of b52b34a (k n aba)
  ...
  const ControlsModal = () => {
<<<<<<< HEAD
    // Tiyakin na ang function na ito ay nag-uupdate ng localControls
=======
>>>>>>> parent of b52b34a (k n aba)
...
<<<<<<< HEAD
            {/* FIX: Using localControls for active state and handleLocalControlChange for onChange */}
            <ControlToggle
              label="Submersible Pump"
              icon={Waves}
              active={localControls.pump}
              onChange={(val: boolean) => handleLocalControlChange('pump', val)}
            />
            <ControlToggle
              label="DC Fan"
              icon={Wind}
              active={localControls.fan}
              onChange={(val: boolean) => handleLocalControlChange('fan', val)}
            />
            <ControlToggle
              label="pH Adjustment"
              icon={Droplets}
              active={localControls.phAdjustment}
              onChange={(val: boolean) => handleLocalControlChange('phAdjustment', val)}
            />
            <ControlToggle
              label="Aerator"
              icon={Activity}
              active={localControls.aerator}
              onChange={(val: boolean) => handleLocalControlChange('aerator', val)}
            />
          </div>
          <button
...
>>>>>>> parent of b52b34a (k n aba)
        ...
        <div><h2 className="text-sm font-bold text-gray-900 mb-3 px-1">Critical Metrics</h2><div className="grid grid-cols-2 gap-3"><SensorCard icon={Thermometer} title="Water Temp" value={sensorData.waterTemp} unit="°C" min={20} max={26} color="bg-blue-500" /><SensorCard icon={Droplets} title="pH Level" value={sensorData.ph} unit="" min={6.5} max={7.5} color="bg-purple-500" /><SensorCard icon={Activity} title="Dissolved O₂" value={sensorData.dissolvedO2} unit="mg/L" min={5} max={8} color="bg-green-500" /><SensorCard icon={Zap} title="Light Level" value={sensorData.lightIntensity} unit="lux" min={10000} max={20000} color="bg-yellow-500" /></div></div>
        <div><h2 className="text-sm font-bold text-gray-900 mb-3 px-1">System Metrics</h2><div className="grid grid-cols-2 gap-3"><SensorCard icon={Waves} title="Water Level" value={Math.round(sensorData.waterLevel)} unit="%" min={70} max={100} color="bg-cyan-500" /><SensorCard icon={Gauge} title="Flow Rate" value={sensorData.waterFlow} unit="L/min" min={3} max={6} color="bg-indigo-500" /><SensorCard icon={Wind} title="Humidity" value={sensorData.humidity} unit="%" min={50} max={80} color="bg-sky-500" /><SensorCard icon={Fish} title="Ammonia" value={sensorData.ammonia} unit="ppm" min={0} max={1} color="bg-orange-500" /></div></div>
<<<<<<< HEAD
>>>>>>> parent of ede362e (grow lights in quick controls)
=======
>>>>>>> parent of b52b34a (k n aba)
